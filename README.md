...
- **Global query filters for multi-tenancy** — tenant isolation is enforced at the EF Core level, so no query can accidentally leak data across tenants regardless of the use case layer.
- **Domain-enforced state machine** — invoice transitions live in the domain entity, not in a service or controller, so the business rule is impossible to bypass.
- **Background job over event-driven** — a daily hosted service keeps the billing cadence simple and observable without introducing a message broker for a demo-scale project.
